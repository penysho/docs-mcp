...
export class GoogleDocsService implements IGoogleDocsService {
  ...
  async readDocumentContent(documentId: string): Promise<string> {
    ...
    try {
      ...
      throw new DocumentError(`ドキュメントの読み取りに失敗しました: ${error instanceof Error ? error.message : String(error)}`);
    }
  }
  ...
  async createNewDocument(title: string, content: string): Promise<string> {
    ...
    try {
      ...
      throw new DocumentError(`ドキュメントの作成に失敗しました: ${error instanceof Error ? error.message : String(error)}`);
    }
  }
  ...
  async updateDocumentContent(documentId: string, content: string, startPosition?: number, endPosition?: number): Promise<void> {
    ...
    try {
      ...
      throw new DocumentError(`ドキュメントの更新に失敗しました: ${error instanceof Error ? error.message : String(error)}`);
    }
  }
   ...
   * @returns 検索結果
   */
  async searchForDocuments(query: string, maxResults: number = 10): Promise<DocumentSearchResult[]> {
    if (!this.authService.isAuthorized()) {
      await this.authService.authorize();
    }
    ...
    try {
      ...
      for (const file of files) {
        if (file.id) {
          try {
            ...
            this.logger.warn(`ドキュメント ${file.id} の詳細取得に失敗しました:`, error);
          }
        }
      }
      ...
      throw new ApiError(`ドキュメントの検索に失敗しました: ${error instanceof Error ? error.message : String(error)}`);
    }
  }

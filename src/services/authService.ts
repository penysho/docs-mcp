...
export class AuthService implements IAuthService {
  ...
  async authorize(): Promise<void> {
    try {
      ...
      if (fs.existsSync(config.googleApi.tokenPath)) {
        // 既存のトークンを読み込む
        const token = JSON.parse(fs.readFileSync(config.googleApi.tokenPath, 'utf8'));
      ...
      throw new AuthError(`Google APIの認証に失敗しました: ${error instanceof Error ? error.message : String(error)}`);
    }
  }
  ...
  getOAuth2Client(): OAuth2Client {
    if (!this.oAuth2Client) {
      throw new AuthError('OAuth2クライアントが初期化されていません。authorize()を先に呼び出してください。');
    }
  ...
  private async getNewToken(): Promise<void> {
    ...
    try {
      ...
      throw new AuthError(`認証コードが無効です: ${error instanceof Error ? error.message : String(error)}`);
    }
  }
} 
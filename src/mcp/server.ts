...
import { IMcpServer, IServiceContainer, IToolRegistry } from '../core/interfaces.js';
...
export class GoogleDocsMcpServer implements IMcpServer {
  ...
  async start(): Promise<void> {
    if (!this.mcpServer) {
      throw new Error('サーバーが初期化されていません。initialize()を先に呼び出してください。');
    }
  ...
  private setupProcessHandlers(): void {
    ...
    // SIGTERMシグナルの処理
    process.on('SIGTERM', () => {
      this.logger.info('SIGTERMシグナルを受信しました。サーバーを停止します...');
      this.gracefulShutdown();
    });
